...
const AudiologistSignupScreen = ({ navigation, route }) => {
  // Audiologist form state
  const [audiologistForm, setAudiologistForm] = useState({
    name: '',
    age: '',
    gender: '',
    phone: '',
  ...
  React.useEffect(() => {
    if (route.params?.returnFromDocUpload) {
      ...
      if (route.params?.preservedFormData) {
        setAudiologistForm(route.params.preservedFormData);
      }
    }
  ...
  // Handle state selection for Audiologist
  const handleStateChange = (state) => {
    setAudiologistForm(prev => ({
      ...prev,
      state,
  ...
  const isSelectedStateUT = audiologistForm.state ? isUnionTerritory(audiologistForm.state) : false;
  
  // Get districts for selected state
  const districtsForSelectedState = audiologistForm.state ? statesAndDistricts[audiologistForm.state] || [] : [];
  
  // Handle Audiologist form input changes
  const handleAudiologistInputChange = (field, value) => {
    setAudiologistForm(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  // Handle Audiologist expertise changes
  const handleAudiologistExpertiseChange = (expertiseType, value) => {
    setAudiologistForm(prev => ({
      ...prev,
      expertise: {
        ...prev.expertise,
  ...
  // Handle language selection for Audiologist
  const handleAudiologistLanguageToggle = (language) => {
    setAudiologistForm(prev => {
  ...
  const handleSubmit = () => {
    ...
    // Validate name
    if (!audiologistForm.name.trim()) {
      newErrors.name = 'Please fill your name';
      isValid = false;
    }
    
    // Validate age
    if (!audiologistForm.age) {
      newErrors.age = 'Please fill your age';
      isValid = false;
    } else if (isNaN(audiologistForm.age) || parseInt(audiologistForm.age) <= 0) {
      newErrors.age = 'Please enter a valid age';
      isValid = false;
    }
    
    // Validate gender
    if (!audiologistForm.gender) {
      newErrors.gender = 'Please select your gender';
      isValid = false;
    }
    
    // Validate phone
    if (!audiologistForm.phone) {
      newErrors.phone = 'Please fill your phone number';
      isValid = false;
    } else if (audiologistForm.phone.length !== 10 || !/^\d+$/.test(audiologistForm.phone)) {
      newErrors.phone = 'Please enter a valid 10-digit phone number';
      isValid = false;
    }
    
    // Validate email
    if (!audiologistForm.email) {
      newErrors.email = 'Please fill your email';
      isValid = false;
    } else if (!isValidEmail(audiologistForm.email)) {
      newErrors.email = 'Please enter a valid email address';
      isValid = false;
    }
    
    // Validate password
    if (!audiologistForm.password) {
      newErrors.password = 'Please fill your password';
      isValid = false;
    } else if (audiologistForm.password.length < 6) {
      newErrors.password = 'Password must be at least 6 characters';
      isValid = false;
    }
    
    // Validate state
    if (!audiologistForm.state) {
      newErrors.state = 'Please select your state';
      isValid = false;
    }
    
    // Validate district (only if state is not a union territory)
    if (!isSelectedStateUT && !audiologistForm.district) {
      newErrors.district = 'Please select your district';
      isValid = false;
    }
    
    // Validate languages
    if (audiologistForm.languagesKnown.length === 0) {
      newErrors.languagesKnown = 'Please select at least one language';
      isValid = false;
    }
    
    // Validate other language if selected
    if (audiologistForm.languagesKnown.includes('Other') && !audiologistForm.otherLanguage.trim()) {
      newErrors.otherLanguage = 'Please specify the other language';
      isValid = false;
    }
    
    // Validate designation
    if (!audiologistForm.designation) {
      newErrors.designation = 'Please select your designation';
      isValid = false;
    }
    
    // Validate semester format if provided (but not required)
    if (audiologistForm.designation === 'Student' && audiologistForm.semester.trim() && (isNaN(audiologistForm.semester) || parseInt(audiologistForm.semester) <= 0)) {
      newErrors.semester = 'Please enter a valid semester number';
      isValid = false;
    }
    
    // Validate qualification
    if (!audiologistForm.qualification) {
      newErrors.qualification = 'Please select your qualification';
      isValid = false;
    }
    
    // Validate other qualification if selected
    if (audiologistForm.qualification === 'Others' && !audiologistForm.otherQualification.trim()) {
      newErrors.otherQualification = 'Please specify your qualification';
      isValid = false;
    }
    
    // Validate expertise (at least one must be selected)
    const hasExpertise = Object.values(audiologistForm.expertise).some(value => value === true);
    if (!hasExpertise) {
      newErrors.expertise = 'Please select at least one area of expertise';
      isValid = false;
    }
    
    // Validate other expertise if Others is selected
    if (audiologistForm.expertise.others && !audiologistForm.otherExpertise.trim()) {
      newErrors.otherExpertise = 'Please specify your expertise';
      isValid = false;
    }
    
    // Validate timings
    if (!audiologistForm.timings.trim()) {
    ...
    navigation.navigate('DocumentUpload', { 
      userType: 'AUDIOLOGIST', 
      formData: audiologistForm,
      returnScreen: 'AudiologistSignup',
    });
  };
    ...
    >
      ...
      >
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>Audiologist Registration</Text>
          <View style={{ width: 24 }} />
        </View>
        ...
        <View style={styles.formContainer}>
          ...
          <View style={styles.formSection}>
            <Text style={styles.sectionTitle}>Basic Information <Text style={styles.requiredStar}>*</Text></Text>
            ...
            <View style={styles.inputContainer}>
              ...
              <TextInput
                ...
                value={audiologistForm.name}
                onChangeText={(text) => {
                  handleAudiologistInputChange('name', text);
            ...
            <View style={styles.inputContainer}>
              ...
              <TextInput
                ...
                value={audiologistForm.age}
                onChangeText={(text) => {
                  handleAudiologistInputChange('age', text);
            ...
            <View style={styles.inputContainer}>
              ...
              <View style={styles.radioGroup}>
                <TouchableOpacity 
                  style={[styles.radioButton, audiologistForm.gender === 'Male' && styles.radioButtonSelected]}
                  onPress={() => {
                    handleAudiologistInputChange('gender', 'Male');
                    if (errors.gender) setErrors({...errors, gender: ''});
                  }}
                >
                  <View style={[styles.radioCircle, audiologistForm.gender === 'Male' && styles.radioCircleSelected]} />
                  <Text style={styles.radioText}>Male</Text>
                </TouchableOpacity>
                
                <TouchableOpacity 
                  style={[styles.radioButton, audiologistForm.gender === 'Female' && styles.radioButtonSelected]}
                  onPress={() => {
                    handleAudiologistInputChange('gender', 'Female');
                    if (errors.gender) setErrors({...errors, gender: ''});
                  }}
                >
                  <View style={[styles.radioCircle, audiologistForm.gender === 'Female' && styles.radioCircleSelected]} />
                  <Text style={styles.radioText}>Female</Text>
                </TouchableOpacity>
                
                <TouchableOpacity 
                  style={[styles.radioButton, audiologistForm.gender === 'Other' && styles.radioButtonSelected]}
                  onPress={() => {
                    handleAudiologistInputChange('gender', 'Other');
                    if (errors.gender) setErrors({...errors, gender: ''});
                  }}
                >
                  <View style={[styles.radioCircle, audiologistForm.gender === 'Other' && styles.radioCircleSelected]} />
            ...
            <View style={styles.inputContainer}>
              ...
              <TextInput
                ...
                maxLength={10}
                value={audiologistForm.phone}
                onChangeText={(text) => {
                  handleAudiologistInputChange('phone', text.replace(/[^0-9]/g, ''));
            ...
            <View style={styles.inputContainer}>
              ...
              <TextInput
                ...
                autoCapitalize="none"
                value={audiologistForm.email}
                onChangeText={(text) => {
                  handleAudiologistInputChange('email', text);
            ...
            <View style={styles.inputContainer}>
              ...
              <View style={[styles.passwordContainer, errors.password ? styles.inputError : null]}>
                <TextInput
                  ...
                  value={audiologistForm.password}
                  onChangeText={(text) => {
                    handleAudiologistInputChange('password', text);
          ...
          <View style={styles.formSection}>
            ...
            <View style={styles.languageContainer}>
              {indianLanguages.map(language => (
                <TouchableOpacity 
                  key={language}
                  style={[styles.languageChip, audiologistForm.languagesKnown.includes(language) && styles.languageChipSelected]}
                  onPress={() => {
                    handleAudiologistLanguageToggle(language);
                    if (errors.languagesKnown) setErrors({...errors, languagesKnown: ''});
                  }}
                >
                  <Text style={[styles.languageChipText, audiologistForm.languagesKnown.includes(language) && styles.languageChipTextSelected]}>
                    {language}
                  </Text>
                </TouchableOpacity>
              ))}
            ...
            {audiologistForm.languagesKnown.includes('Other') && (
              <View style={styles.inputContainer}>
                ...
                <TextInput
                  ...
                  value={audiologistForm.otherLanguage}
                  onChangeText={(text) => {
                    handleAudiologistInputChange('otherLanguage', text);
          ...
          <View style={styles.formSection}>
            ...
            <View style={styles.inputContainer}>
              <SearchableDropdown
                ...
                selectedItem={audiologistForm.state}
                onItemSelect={(state) => {
            ...
            <View style={styles.inputContainer}>
              <SearchableDropdown
                ...
                selectedItem={audiologistForm.district}
                onItemSelect={(district) => {
                  handleAudiologistInputChange('district', district);
                  if (errors.district) setErrors({...errors, district: ''});
                }}
                disabled={isSelectedStateUT || !audiologistForm.state}
          ...
          <View style={styles.formSection}>
            ...
            <View style={styles.radioGroup}>
              {designationOptions.map(option => (
                <TouchableOpacity 
                  key={option}
                  style={[styles.radioButton, audiologistForm.designation === option && styles.radioButtonSelected]}
                  onPress={() => {
                    handleAudiologistInputChange('designation', option);
                    if (errors.designation) setErrors({...errors, designation: ''});
                  }}
                >
                  <View style={[styles.radioCircle, audiologistForm.designation === option && styles.radioCircleSelected]} />
          ...
          <View style={styles.formSection}>
            ...
            <View style={styles.qualificationContainer}>
              <View style={styles.qualificationRow}>
                {qualificationOptions.slice(0, 3).map(option => (
                  <TouchableOpacity 
                    key={option}
                    style={[styles.radioButton, audiologistForm.qualification === option && styles.radioButtonSelected]}
                    onPress={() => {
                      handleAudiologistInputChange('qualification', option);
                      if (errors.qualification) setErrors({...errors, qualification: ''});
                    }}
                  >
                    <View style={[styles.radioCircle, audiologistForm.qualification === option && styles.radioCircleSelected]} />
                    <Text style={styles.radioText}>{option}</Text>
                  </TouchableOpacity>
                ))}
              </View>
              <View style={styles.qualificationRow}>
                <TouchableOpacity 
                  key="Others"
                  style={[styles.radioButton, audiologistForm.qualification === 'Others' && styles.radioButtonSelected]}
                  onPress={() => {
                    handleAudiologistInputChange('qualification', 'Others');
                    if (errors.qualification) setErrors({...errors, qualification: ''});
                  }}
                >
                  <View style={[styles.radioCircle, audiologistForm.qualification === 'Others' && styles.radioCircleSelected]} />
            ...
            {audiologistForm.designation === 'Student' && (
              <View style={[styles.inputContainer, { marginTop: 12 }]}>
                ...
                <TextInput
                  ...
                  keyboardType="numeric"
                  value={audiologistForm.semester}
                  onChangeText={(text) => {
                    handleAudiologistInputChange('semester', text);
            ...
            {audiologistForm.qualification === 'Others' && (
              <View style={[styles.inputContainer, { marginTop: 12 }]}>
                ...
                <TextInput
                  ...
                  value={audiologistForm.otherQualification}
                  onChangeText={(text) => {
                    handleAudiologistInputChange('otherQualification', text);
          ...
          <View style={styles.formSection}>
              ...
              return (
                <View key={option} style={styles.checkboxContainer}>
                  <TouchableOpacity 
                    ...
                    onPress={() => {
                      handleAudiologistExpertiseChange(key, !audiologistForm.expertise[key]);
                      if (errors.expertise) setErrors({...errors, expertise: ''});
                    }}
                  >
                    <View style={[styles.checkboxBox, audiologistForm.expertise[key] && styles.checkboxBoxChecked]}>
                      {audiologistForm.expertise[key] && <Ionicons name="checkmark" size={16} color={COLORS.white} />}
                    </View>
            ...
            {/* Other Expertise Input Field */}
            {audiologistForm.expertise.others && (
              <View style={styles.inputContainer}>
                <Text style={styles.inputLabel}>Specify Other Expertise <Text style={styles.requiredStar}>*</Text></Text>
                <TextInput
                  style={[styles.input, errors.otherExpertise ? styles.inputError : null]}
                  placeholder="Please specify your expertise"
                  value={audiologistForm.otherExpertise}
                  onChangeText={(text) => {
                    setAudiologistForm(prev => ({
                      ...prev,
                      otherExpertise: text
                    }));
                    if (errors.otherExpertise) setErrors({...errors, otherExpertise: ''});
                  }}
                />
                {errors.otherExpertise ? (
                  <View style={styles.fieldErrorContainer}>
                    <Ionicons name="alert-circle" size={16} color={COLORS.error} />
                    <Text style={styles.fieldErrorText}>{errors.otherExpertise}</Text>
                  </View>
                ) : null}
              </View>
            )}
          ...
          <View style={styles.formSection}>
            ...
            <TextInput
              ...
              multiline
              numberOfLines={3}
              value={audiologistForm.timings}
              onChangeText={(text) => {
                handleAudiologistInputChange('timings', text);
...
export default AudiologistSignupScreen;
